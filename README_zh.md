...
[📖 在线演示](https://thysrael.github.io/Horizon/) · [📋 配置指南](https://thysrael.github.io/Horizon/configuration) · [🧭 架构与流程图](docs/architecture.md) · [English](README.md)
...
默认 RSS 源保持小而精：Simon Willison、OpenAI News、Google DeepMind Blog、Hugging Face Blog、GitHub Blog、GitHub Changelog、Latent Space、Import AI、Microsoft AI Blog 和 GitHub Trending。建议后续逐步增加 RSS，避免低质量转载把日报挤满。

如果希望每天早晚自动运行并把 Markdown 同步到 iCloud 目录，可以使用 `scripts/run-and-export-icloud.sh` 和 launchd 模板。导出文件默认按日期写入 `daily/YYYY-MM-DD/`，并使用唯一文件名避免覆盖；`latest.md` 和 `posts/` 导出默认关闭，需要时可用环境变量开启。详细步骤见 [iCloud 早晚报计划](docs/icloud-schedule.md)。
